...
// ── RECRUITER MODE ──────────────────────────────────────────────────────────
function RecruiterPage({ onAI, darkMode, toggleDark }: { onAI: () => void; darkMode: boolean; toggleDark: () => void }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: bg, color: fg, fontFamily: "'Space Grotesk', sans-serif" }}>
      ...
      <button onClick={toggleDark} style={{
        ...
        border: `1px solid ${bdr}`, color: sub, background: bg,
      }}>
        {darkMode ? '☀ Light' : '◑ Dark'}
      </button>

      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '5rem 2rem 5rem' }}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
          style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: '1px', background: bdr, marginBottom: '5rem' }}>
          {[
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} style={{ marginBottom: '5rem' }}>
          <h2 style={{ fontSize: '1.25rem', fontFamily: "'Playfair Display',serif", fontWeight: 300, paddingBottom: '0.75rem', borderBottom: `1px solid ${bdr}`, marginBottom: 0 }}>Selected Projects</h2>
          {[
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
          <h2 style={{ fontSize: '1.25rem', fontFamily: "'Playfair Display',serif", fontWeight: 300, paddingBottom: '0.75rem', borderBottom: `1px solid ${bdr}`, marginBottom: '1rem' }}>Technical Skills</h2>
...
// ── FRIEND MODE ─────────────────────────────────────────────────────────────
function FriendPage({ onAI, darkMode, toggleDark }: { onAI: () => void; darkMode: boolean; toggleDark: () => void }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: bg, color: fg, fontFamily: "'Space Grotesk', sans-serif" }}>
      ...
      <button onClick={toggleDark} style={{
        position: 'fixed', top: '70px', right: '16px', zIndex: 100,
        fontSize: '11px', padding: '4px 12px', borderRadius: '999px', cursor: 'pointer',
        border: `1px solid ${bdr}`, color: sub, background: card,
      }}>
        {darkMode ? '☀' : '◑'}
      </button>

      <main style={{ maxWidth: '560px', margin: '0 auto', padding: '5rem 1.5rem 5rem' }}>
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }}>
          <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
            ...
            <h1 style={{ fontSize: '2rem', fontFamily: 'Georgia,serif', fontWeight: 400, marginBottom: '0.5rem' }}>Hey, I'm Rafsan!</h1>
            <p style={{ lineHeight: 1.6, color: sub }}>Student, builder, occasional poet. I love making things — from apps to physics experiments!</p>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '2rem' }}>
            {[
              ...
              { e: '🌱', t: 'AgriBase',   b: 'Bronze medal. 50,000+ records.',          bg: darkMode ? '#001020' : '#eff6ff', bd: darkMode ? '#002244' : '#93c5fd' },
              { e: '⚡', t: '297 Cadets', b: 'Led the whole college as Prefect.',       bg: darkMode ? '#1a0020' : '#fdf4ff', bd: darkMode ? '#440050' : '#d8b4fe' },
            ].map(c => (
          ...
          <div style={{ borderRadius: '16px', padding: '1.25rem', marginBottom: '2rem', background: card, border: `1.5px solid ${bdr}` }}>
            ...
            {[['💻','Building this portfolio'],['📚','Studying algorithms & DS'],['🌍','Planning uni in Türkiye'],['☕','Surviving on tea']].map(([e,t]) => (
...
function DeveloperPage({ onAI }: { onAI: () => void }) {
  ...
  return (
    <>
      <WebGLBackground />
      <NavBar />
      <main className="relative z-10 min-h-screen px-4 pb-16 max-w-6xl mx-auto pt-24">
        ...
        <div className="text-center text-xs tracking-widest border-t pt-8 font-mono" style={{ color: '#8888aa', borderColor: '#1a1a2e' }}>
          Press <kbd className="px-1 rounded" style={{ background: '#0d0d12', border: '1px solid #1a1a2e' }}>` </kbd> terminal &nbsp;·&nbsp;
          <kbd className="px-1 rounded" style={{ background: '#0d0d12', border: '1px solid #1a1a2e' }}>⌘K</kbd> commands
        </div>
      </main>
    </>
  );
...
export default function HomePage() {
  ...
  return (
    <>
      {/* No floating mode switcher — it lives in the NavBar now */}
      {mode === 'developer' && <DeveloperPage onAI={() => setShowAI(true)} />}
      {mode === 'recruiter' && (
        <>
          <NavBar />
          <RecruiterPage onAI={() => setShowAI(true)} darkMode={darkMode} toggleDark={toggleDark} />
        </>
      )}
      {mode === 'friend' && (
        <>
          <NavBar />
          <FriendPage onAI={() => setShowAI(true)} darkMode={darkMode} toggleDark={toggleDark} />
        </>
      )}
